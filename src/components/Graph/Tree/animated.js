...
export default class Animated extends React.PureComponent{
    ...
    getClosestAncestor(node, stateWithNode, stateWithoutNode) {
        ...
        let f1 = n => this.areNodesSame(oldParent, n);
        let f2 = c => this.areNodesSame(oldParent, c);

        while(oldParent) {
            let newParent = stateWithoutNode.nodes.find(f1);

            if (newParent) {
                return newParent;
            }

            oldParent = stateWithNode.nodes.find(n => (this.props.getChildren(n) || []).some(f2));
        }
